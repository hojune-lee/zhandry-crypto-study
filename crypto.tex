...
This is study note of Prof. Zhandry's lecture\cite{zhandry_crypto}. 
...
Means that we can choose good $x$ via sampling with non-negligible probability. Now suppose that given $x$ is good. 
Then the goal of adversary $C$ is breaking OWF assumption of $F$. If the probability that given $x$ is good is non-negligible and 
if $x$ is good then $C$ can compute $x$ from $F(x)$ with non-negligible probability, then we can say that $C$ has non-negligible advantage to break OWF assumption of $F$. 
Now $B$ break OWF of $F$ with good $x$. Let's define 
\[H(r) = B(r, F(x))\] 
Choose enoughly many $r \xleftarrow{\$} \{0,1\}^n$ and $C$ computes 
\[H(r) \oplus H(r \oplus e_i)\]
Then, if $B$ computes both of them correctly, 
\[H(r) \oplus H(r \oplus e_i) = \langle x, r \rangle \oplus \langle x, r \oplus e_i \rangle = \langle x, e_i \rangle = x_i\]
So, $C$ can obtain $x_i$ correctly when $B$ guess both of them correctly. So, 
\begin{align*} 
    \Pr[C \text{ correctly guess } x_i ~|~ x \text{ is good }] &= \Pr[H(r) \oplus H(r \oplus e_i) = x_i] \\
    & = \Pr[B \text{ guess both correctly }] \\ 
    & = 1 - \Pr[B \text{ wrong one of them }] \\
    & \geq 1 - 2\left(\frac{1}{4} - \frac{\gamma}{2}\right) \\
    & = \frac{1}{2} + \gamma
\end{align*}

So, 
\begin{align*}
    \Pr[F(C(F(x))) = F(x)~|~ x \xleftarrow{\$} \{0,1\}^n] & \geq \Pr[C \text{ correct}~|~ x \text{ is good}]\Pr[x \text{ is good}] \\
    & \geq \frac{1}{2} + \CO(\gamma)
\end{align*}
$C$ has non-negligible advantage to break OWF assumption of $F$, which is contradiction. 

\end{proof}

\chapter{Statistical Distance and Entropy}

In this chapter, I reference Alex Lombardi's note\cite{lombardi_crypto}
